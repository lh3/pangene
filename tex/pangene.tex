...
with $E_g=\big\{\{(v,{\rm s}),(v,{\rm e})\}|v\in V\big\}$ and $E_l$ being the set of unordered connections between starts or ends of genes in $Z$.
Fig.~\ref{fig:exa1}d shows an example.
A \emph{net graph} $G_N$ is constructed by contracting all edges in $E_l$.
As a result, vertices in $G_N$ are components connected by edges in $E_l$
and edges represent genes (Fig.~\ref{fig:exa1}e and Fig.~\ref{fig:inv}e).

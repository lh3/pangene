...
We identified 88 generalized bubbles in the graph,
involving 655 protein coding genes affected by gene copy, gene order or gene orientation changes between the two genome.
...
and less than one second to identify 251 generalized bibubbles in the graph.
Many of the bibubbles were supported by one contig only.
When we pruned edges supported by one contig,
the resultant graph contained 198 generalized bibubbles involving 896 protein-coding genes.
These include manually confirmed gene-level variations around {\it CYP2D6}/{\it CYP2D7}, {\it C4A}/{\it C4B} and 
{\it RHD}/{\it RHCE}~\citep{Liao:2023aa}.

We next investigated genes with presence/absence variants (PAVs).
There were 18,262 genes on the autosomes of GRCh38.
4.1\% of them were not present in all haplotypes and 0.4\% ($n$=69) were only present in $\le$50\% of haplotypes.
Among the 69 genes, for example, \emph{NPY4R} is paralogous to \emph{NPY4R2} which differs at one amino acid.
On most haplotypes, \emph{NPY4R2} was aligned better than \emph{NPY4R} and pangene annotated two \emph{NPY4R2} genes but no \emph{NPY4R}.
Therefore \emph{NPY4R} became a PAV.
When we excluded genes having paralogs of $\ge$95\% identity from the list,
only 18 were left.
This list includes three KIR genes and \emph{HLA-DRB4} that are known to be PAVs.
We checked a few remaining genes in the list.
\emph{PRH1}, \emph{LCE3B} and \emph{LCE3C} are in relatively simple regions and look like real PAVs.
\emph{KCND2} has an intron longer than 200 kb, which caused miniprot misalignment.
\emph{GSTT2} has a close paralog \emph{GSTT2B} that CD-HIT failed to identify -- it seems an algorithm glitch.
The flanking region around \emph{NOTCH2NLR} is complex and was poorly assembled in most haplotypes.
This may explain the low occurrence rate of this gene.
Overall, 99.9\% of human genes or their close paralogs were present in the majority of human haplotypes.
It is rare to see a protein-coding gene completely missing without paralogs as backups.

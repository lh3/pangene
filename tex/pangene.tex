...
Similarly, ``{\tt A}'' and ``{\tt B}'' are cycle equivalent in Fig.~\ref{fig:complex}c but they do not bracket generalized bibubbles.

Pangene alleviates this issue by introducing a dummy vertex $p_0$ to the net graph $G_N$.
$p_0$ connects all vertices in $G_N$ with degree one and, in the presence of a reference genome,
also connects the genes at the ends of each chromosome.
This way, ``{\tt C}'' and ``{\tt D}'' are no longer cycle equivalent in Fig.~\ref{fig:exa1}g
and ``{\tt A}'' and ``{\tt B}'' are not equivalent in Fig.~\ref{fig:complex}d.
However, some cycle equivalent edges still do not correspond to bibubbles, such as ``{\tt B}'' and ``{\tt E}'' in Fig.~\ref{fig:complex}f.
Pangene still needs to check if two equivalent edges in $G_N$ enclose a bibubble in $G_B$.

To find generalized bibubbles in $G_B$,
pangene converts $G_B$ to $G_N$,
identifies all single-entry-single-exit (SESE) regions in $G_N$ with an algorithm by \citet{DBLP:conf/pldi/JohnsonPP94},
test each identified gene pairs in $G_B$ using the definition of generalized bibubbles,
and then outputs them.
The first two steps can be done in $O(|V|+|E|)$ time.
When testing bibubbles,
pangene traverses up to $m=100$ genes.
The total time complexity is $O(|V|+|E|+b\cdot m)$ where $b$ is the number of SESE regions.
We emphasize that this algorithm is approximate.
Although it does not identify false bibubbles by testing every SESE,
it in theory might miss bibubbles if the dummy vertex in $G_N$ is wrongly placed.

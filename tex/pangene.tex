...
with $E_g=\big\{\{(v,{\rm s}),(v,{\rm e})\}|v\in V\big\}$ and $E_l$ being the set of undirected connections between starts or ends of genes in $Z$.
Fig.~\ref{fig:exa1}d shows an example.
A valid walk in a biedged graph cannot pass two consecutive edges in $E_l$.
Due to this restriction, many classical algorithms in graph theory cannot be easily applied to a biedged graph.
...
and an edge represents a gene in $V$ with $|E_n|=|V|$ (Fig.~\ref{fig:exa1}e and Fig.~\ref{fig:inv}e).
A net graph is an undirected graph that is not equivalent to the bidirected graph $G_B$.
...
Nonetheless, Fig.~\ref{fig:exa1}f still implies a bubble-like subgraph even with the loss of directionality.
This is the base of our approximate algorithm.

...
int32_t pg_filter_full_shadow(const pg_opt_t *opt, int32_t n_gene, const pg_prot_t *prot, pg_genome_t *g)
{
	...
	for (i = 1, i0 = 0; i < g->n_hit; ++i) {
		...
		for (j = i0; j < i; ++j) {
			...
			if (aj->flt) continue;
			gj = prot[aj->pid].gid;
			if (gi == gj) continue; // only deal with different genes
			...
			if (ai->score2 < aj->score2 && (uint64_t)ai->mlen * aj->blen < (uint64_t)aj->mlen * ai->blen)
				flt[i] = 1;
			else if (ai->score2 > aj->score2 && (uint64_t)ai->mlen * aj->blen > (uint64_t)aj->mlen * ai->blen)
				flt[j] = 1;
		}
	}
	kept = PG_CALLOC(int8_t, n_gene);
	for (i = 0; i < g->n_hit; ++i) {
		const pg_hit_t *a = &g->hit[i];
		if (!a->flt && !flt[i])
			kept[prot[a->pid].gid] = 1;
	}
	for (i = 0; i < g->n_hit; ++i) {
		pg_hit_t *a = &g->hit[i];
		if (a->flt) continue;
		if (!kept[prot[a->pid].gid])
			a->flt = a->flt_all_shadow = 1, ++n_flt;
	}
	free(kept);
	free(flt);
	return n_flt;

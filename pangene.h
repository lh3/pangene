...
#define PG_VERSION "0.0-r123-dirty"
...
typedef struct {
	const char *name;
	int32_t len;
	uint32_t gid:30, flt:1, rep:1; // rep: if this protein is the representative isoform of the gene
	int32_t n, avg_score2;
} pg_prot_t;

typedef struct {
	const char *name;
	uint32_t len:31, flt:1; // longest ORF or representative ORF
...
typedef struct {
	...
	int32_t pid_dom;
	uint32_t rev:1, flt:1, flt_iso_ov:1, flt_iso_scat:1, flt_all_shadow:1, pseudo:1, vtx:1, overlap:1, shadow:1, rep:1, weak_br:2;
	int64_t cs, cm, ce;
} pg_hit_t;

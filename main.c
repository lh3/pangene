...
static int32_t pg_usage(FILE *fp, const pg_opt_t *opt)
{
	...
	fprintf(fp, "    -j            filter pseudogenes across samples\n");
...
int main(int argc, char *argv[])
{
	...
	pg_opt_init(&opt);
	while ((c = ketopt(&o, argc, argv, 1, "d:e:l:f:g:p:b:B:y:Fr:c:a:wv:GD:C:T:X:I:P:m:JjOS", long_options)) >= 0) {
		// input options
		...
		else if (c == 'j') opt.flag |= PG_F_JOINT_PSEUDO;
		else if (c == 'J') opt.flag &= ~PG_F_JOINT_PSEUDO; // deprecated
